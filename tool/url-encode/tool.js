const URLTool = {
    ...
    encode() {
        ...
        if (!text) { alert('请输入要编码的URL'); return; }
        try {
            this.outputText.value = encodeURIComponent(text);
        } catch (e) { alert('编码失败: ' + e.message); }
    },
    
    decode() {
        const text = this.inputText.value;
        if (!text) { alert('请输入要解码的URL'); return; }
        try {
            this.outputText.value = decodeURIComponent(text);
    ...
    clear() { this.inputText.value = ''; this.outputText.value = ''; }
};

document.addEventListener('DOMContentLoaded', () => URLTool.init());
